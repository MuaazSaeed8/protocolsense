...
export const App: React.FC = () => {
  ...
  const [guestAnalysisDone, setGuestAnalysisDone] = useState(false);
  ...
  const handleAnalyze = async () => {
    ...
    if (!user && guestAnalysisDone) {
      setToast("Sign in to run more analyses — it's free.");
      return;
    }
    ...
    try {
      ...
      setStatus(AnalysisStatus.SUCCESS);
      setMobileTab('results');
      if (!user) setGuestAnalysisDone(true);
  ...
  if (!user && !guestMode) {
    return (
      <div className="flex flex-col bg-background text-on-surface font-sans min-h-screen relative">
        ...
        <LandingPage onSignIn={handleSignIn} onTryFree={() => setGuestMode(true)} />
      </div>
    );
  }
        ...
        <section className={`flex-1 overflow-x-hidden bg-surface-container/30 px-4 lg:px-10 py-6 lg:py-10 space-y-8 lg:rounded-tl-[48px] relative lg:overflow-y-auto custom-scrollbar ${mobileTab === 'results' ? 'block' : 'hidden lg:block'}`}>
          ...
          {status === AnalysisStatus.SUCCESS && result && (
            <div id="tour-analysis-area" className="space-y-10 lg:space-y-16">
              {!user && guestAnalysisDone && (
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 bg-surface-container border border-primary/20 rounded-2xl px-5 py-4 animate-in fade-in slide-in-from-top-2 duration-500">
                  <div className="space-y-0.5">
                    <p className="text-sm font-bold text-on-surface">Sign in to save your results and get 25 analyses per day</p>
                    <p className="text-xs text-on-surface-variant/70">Your current results won't be lost.</p>
                  </div>
                  <button
                    onClick={handleSignIn}
                    className="flex items-center gap-2 bg-primary text-background px-4 py-2 rounded-full text-sm font-bold hover:opacity-90 transition-all shrink-0 shadow-sm shadow-primary/20"
                  >
